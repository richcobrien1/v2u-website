      ...
      <div className="p-6 pt-24">
        <div className="max-w-4xl mx-auto">
          <div className="rounded-xl bg-gray-100 dark:bg-gray-800 p-6 shadow-lg">
            <h1 className="text-2xl dark:text-white mb-4">Subscribers</h1>
            <div className="mb-4">
            ...
            <div className="mb-4 flex gap-2">
              <input value={newEmail} onChange={(e) => setNewEmail(e.target.value)} className="p-2 border border-gray-300 dark:border-gray-600 rounded w-full bg-white dark:bg-gray-700 dark:text-white" placeholder="email@example.com" />
            ...
            {loading ? <p className="text-gray-700 dark:text-gray-300">Loading...</p> : (
              <table className="w-full table-auto">
                <thead>
                  <tr className="text-left dark:text-white"><th>Email</th><th>Created</th><th></th></tr>
                </thead>
                <tbody>
                  {subs.map(s => (
                    <tr key={s.email} className="border-t border-gray-300 dark:border-gray-600">
                      <td className="dark:text-white">
                        {editing === s.email ? (
                          <input value={editValue} onChange={(e) => setEditValue(e.target.value)} className="p-1 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 dark:text-white" />
                        ) : (
                          s.email
                        )}
                      </td>
